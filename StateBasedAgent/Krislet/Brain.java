...
//    Date:             October 4, 2021
//    Reason:			Implemented state-based krislet soccer playing agent.
...
class Brain extends Thread implements SensorInput
{
	...
	// Boolean variable that will help to remember if the goal was seen in the last run of the while loop
	Boolean GoalSeenInPast = false;
	...
	while( !m_timeOver )
		...
		//Boolean variable that will help to remember if the goal was seen in the last run of the while loop
		//Declared prior to the while loop. Updated at the end of each cycle of the while loop to remember if GoalSeenInPast is true in the future.
		if (GoalSeenInPast==true)
		{
			System.out.println("Goal Seen in Past is True");
		}
		else
		{
			System.out.println("Goal Seen in Past is False");
		}
        ...
        if (Action.equals("turn"))
        {
            ...
            {
                if (goal==null && GoalSeenInPast==true)
				{
					m_krislet.turn(90);
					System.out.println("GoalSeenInPast is true. This means the goal is somewhere around me. Search at wider angle 90 deg to find it quickly.");
				}
				else if (goal==null && GoalSeenInPast==false)
				{
		...
		// Setting the flag. Set the boolean variable GoalSeenInPast to false if TempEnvironmentStates.GoalVisibility in the current cycle is "N" (No).
		// This variable will be used later in the future cycle of the while loop
		// And that's how the agent becomes state based as it's able to remember something that happened in the previous cycle of while loop
		if (TempEnvironmentStates.GoalVisibility.equals("N"))
		{
			GoalSeenInPast = false;
		}
		else
		{
			GoalSeenInPast = true;
